...
The prompts are the product. `system.md` is the same shape in both agents: who the agent is, that the session is read-only, the MCP tools it has, how to write StackQL that lands on the first try (key columns in WHERE, booleans as 0/1, JSON columns, fan-out, ORDER BY with LIMIT, the CTE for a third join), and how to report. `task.md` is the job, numbered, naming the resources to read so the agent spends its tool calls on evidence rather than archaeology. Edit either file and rebuild; nothing else changes.
